...
struct FSInterface {// Concurrency support is guaranteed(see note); `errno' in the `struct FSReturn' is used instead of the thread's `errno'
	...
	struct FSReturn (*open)(const char*, int, mode_t, void*);// Arguments: ("fileName", "flags", "mode", "fs"); Returns: "return"; The path passed is guaranteed to be relative to the mount point; The non-error return value is `int' "kfd" (must associate it yourself)
